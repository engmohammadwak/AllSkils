...
def detect_technology(text: str, main_category: str, sub_category: str) -> str:
    ...
    NOTE: sub_category is normalised to lower-case so that values like
    'Backend' (stored in JSON) match the lower-case keys in TECH_STACKS.
    """
    sub_category_lower = sub_category.lower()

    if main_category not in TECH_STACKS:
        return "general"
    if sub_category_lower not in TECH_STACKS[main_category]:
        return "general"

    text_lower = text.lower()
    tech_scores: dict[str, int] = {}

    for tech_name, keywords in TECH_STACKS[main_category][sub_category_lower].items():
        score = 0
        for kw in keywords:
            kw_l = kw.lower()
            score += text_lower.count(kw_l) * 2
